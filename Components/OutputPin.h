...
public:
	...
	int GetConnectionNo();  //Get no. of connections connected to that output pin.
	int Getm_Conn();
	bool IsFull(); //Returns true if m_Conn==m_FanOut indicating that no further connections can be connected to this pin
	Connection* get_m_Connections(int i);
};
