...
{	
	...
	{
		if (x > (UI.width - UI.ToolBarHeight) && x < UI.width) //simulation tool bar
		{
		...
		if (y >= 0 && y < UI.ToolBarHeight) //tool bar (some features in the design tool bar can be used in the simualtion mode)
		{
			//Check whick Menu item was clicked
			//==> This assumes that menu items are lined up horizontally <==
			int ClickedItemOrder = (x / UI.ToolItemWidth);
